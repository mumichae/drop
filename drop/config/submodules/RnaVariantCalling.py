...
class RVC(Submodule):
    ...
    def setBatchDict(self):
        ...
        for key,values in self.rnaIDs.items():
            for v in values:
                if(v) in dict_.keys():
                    raise ValueError("RNA IDs must be unique to the RNA Variant Calling group, can not have an RNA_ID point to multiple RNA_VARIANT_GROUPs")
